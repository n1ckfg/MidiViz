  ...
  Settings(String _s) {
    try {
      ...
      for (int i=0;i<settings.data.length;i++) {
        ...
        if (settings.data[i].equals("MIDI Channel Number")) midiChannelNum = setInt(settings.data[i+1]);
        if (settings.data[i].equals("MIDI Port Number")) midiPortNum = setInt(settings.data[i+1]);
        if (settings.data[i].equals("MIDI Velocity Floor")) velocityFloor = setInt(settings.data[i+1]);
